...
class	Parser {
	...
	bool	parseModeCommand(std::shared_ptr<Client>& client, std::string& input, State& state);
	...
	bool	parseTopicCommand(std::shared_ptr<Client>& client, std::string& input, State& state);
};