...
void WhoCommand::sendReply(const std::shared_ptr<Client>& who_client, const std::string& channel, const std::string& flags) const {
    struct reply whoReply = RPL_WHOREPLY;
    std::string target;

    whoReply.msg.append(who_client->getName());
    target += channel + " " + who_client->getUsername() + " " + who_client->getHostname() + " " + _state.getServerName() \
        + " " + who_client->getNickname() + " " + flags;
    ...
    if (!_channel.empty()) {
        ...
        if (who_client != _state.getClients().end()) {
            ...
            for (auto it = _state.getChannels().begin(); it != _state.getChannels().end(); ++it) {
                if (it->isClient(*who_client) && it->isClient(_client)) {
            ...
            else if (_state.getChannel(shared_channel)->isOperator(*who_client)) {
                flags += "@";
            }
            sendReply(*who_client, shared_channel, flags);
        }
